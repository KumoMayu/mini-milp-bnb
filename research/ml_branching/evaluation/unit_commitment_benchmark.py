...
from research.ml_branching.runtime.inference import LearnedBranchingPolicy, LearnedGNNBranchingPolicy
from research.ml_branching.training.oracle.strong_branching import StrongBranchingPolicy
from research.ml_branching.unit_commitment import generate_from_config, instance_from_parameters
...
def write_report(config: dict, comparison_csv: Path = COMPARISON_CSV) -> Path:
    ...
    dataset = BranchingDataset.from_dir(config.get("dataset_path", "research/ml_branching/data/generated/unit_commitment_round0"))
    ...
    lines += [
        ...
        "## 9. 文件清理与当前结构",
        "",
        "- 保留上一轮最佳 MLP checkpoint：`research/ml_branching/trained_models/learning_branching/run_12_lr_0.001_h_64_seed_3.pt`。",
        "- 本轮有效入口：`research/ml_branching/training/train_gnn.py`、`research/ml_branching/training/dagger.py`、`research/ml_branching/evaluation/unit_commitment_benchmark.py`。",
    ]
