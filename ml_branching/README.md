...
`ml_branching/` 是自写 MILP Branch-and-Bound 求解器的可选研究模块。普通求解主线仍在 `solver/`，默认 `from solver import MILPProblem, solve_milp` 不依赖 Torch，也不会自动加载 GNN。
...
```text
trained_models/learning_branching/
```
...
policy = LearnedGNNBranchingPolicy.from_checkpoint(
    "trained_models/learning_branching/gnn_stability_seed_1.pt"
)
...
```zsh
.venv-ml/bin/python -m ml_branching.training.train_gnn \
  --config ml_branching/configs/stability_train.json
```

本阶段不继续训练模型，不重跑耗时 benchmark。完整解释见 `reports/learning_branching/GNN分支学习过程说明.md`。
